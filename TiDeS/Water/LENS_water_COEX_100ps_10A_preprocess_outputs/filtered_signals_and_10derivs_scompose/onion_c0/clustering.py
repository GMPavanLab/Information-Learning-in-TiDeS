...
def plot_pop_fractions_v2(
    ...
    base_colors = ["purple", "gray", "violet", "yellow", "green", "gray"]
    ...
    axes.set_ylabel("ENVs Population", fontsize=16)
    axes.set_xscale("log")
...
def main() -> None:
    ...
    for i, delta_t in enumerate(all_delta_t):
        ...
        #state_list, labels= apply_threshold_merge(state_list,labels,0.013)
        state_list, labels= apply_threshold_merge(state_list,labels,0.046)
        ...
        if i in [2, 6, 12]:
            ...
            # --- Write info_states.txt ---
            with open("info_states.txt", "w") as f:
                f.write("state_id\tth_inf\tth_sup\n")
                for state_id, state in enumerate(state_list):
                    th_inf = state.get_attributes()["th_inf"]
                    th_sup = state.get_attributes()["th_sup"]
                    state_repr = str(state)  # or state.__repr__() if you want
                    f.write(f"{state_id}\t{th_inf}\t{th_sup}\t{state_repr}\n")
                    
            print("[✔️] info_states.txt saved.")    
            ...
            onion.plot.color_trj_from_xyz(
                "/home/dom/Scrivania/PhD/Python_codes/becchi_clustering/Water_derivative_1_code/10_derivatives+TICA/LENS_WaterCOEX_469frames.xyz", labels, n_particles, delta_t
            )
