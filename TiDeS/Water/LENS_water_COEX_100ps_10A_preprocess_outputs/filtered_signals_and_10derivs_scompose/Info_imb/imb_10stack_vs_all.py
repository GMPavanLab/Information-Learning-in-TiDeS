...
print("\n[INFO] Starting greedy selection...")
...
if os.path.exists(datafile):
    ...
    for step in range(n):
        ...
        print(f"   [✔️] Selected comp {best_candidate} (min imbalance: {best_imb:.5f})\n")

    print("[INFO] Greedy selection complete!")
    print(f"Selected order: {selected}")
    print(f"Min imbalance at each step: {min_imbalances}")

    # Save results for later analysis/plotting
    np.savez(datafile, selected=selected, min_imbalances=min_imbalances, selected_labels=selected_labels)
    print(f"[💾] Saved selection and imbalance trace to {datafile}")

# Optional: plot the imbalance progression
'''
try:
    import matplotlib.pyplot as plt
    plt.figure(figsize=(8, 5))
    plt.plot(range(1, len(min_imbalances)+1), min_imbalances, '-o', c='royalblue')
    plt.xlabel("Selection step")
    plt.ylabel("Minimum imbalance (A→B, B=all)")
    plt.title("Greedy selection: minimum imbalance vs step")
    plt.xticks(range(1, n+1), selected_labels, rotation=45)
    plt.grid(True, linestyle="--", alpha=0.5)
    plt.tight_layout()
    plt.savefig("greedy_min_imbalance_progression.png", dpi=300)
    plt.show()
    print("[📈] Plot saved: greedy_min_imbalance_progression.png")
except Exception as e:
...
try:
    ...
    ax.plot(range(0, len(min_imbalances)), min_imbalances, '-o', c='royalblue')
    ax.set_xlabel("N",fontsize=17)
    ax.set_ylabel(r"$I_{\mathrm{IMB}}\;D^{(0-N)} \rightarrow TiDeS$",fontsize=17)
    #ax.set_title("Greedy selection: minimum imbalance vs step")
    #ax.set_xticks(range(1, len(min_imbalances)+1))
    ...
    plt.savefig("greedy_min_imbalance_progression_v2.png", dpi=300)
    plt.show()
    print("[📈] Plot saved: greedy_min_imbalance_progression_v2.png")
except Exception as e:
    print(f"[⚠️] Could not plot: {e}")    
